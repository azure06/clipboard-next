...
impl Clipboard for ClipboardContext {
	...
	fn get_image(&self) -> Result<RustImageData> {
		if let Some(&cf_png_format) = self.format_map.get(CF_PNG) {
			if clipboard_win::is_format_avail(cf_png_format) {
				let image_raw_data = self.get_buffer(CF_PNG)?;
				return RustImageData::from_bytes(&image_raw_data);
			}
		}

		// Use a function to abstract DIB handling
		fn handle_dib(data: &[u8]) -> Result<RustImageData> {
			let decoder: BmpDecoder<Cursor<&[u8]>> =
				BmpDecoder::new_without_file_header(Cursor::new(data))
					.map_err(|e| format!("BMP decoding error: {}", e))?;
			let dynamic_image = DynamicImage::from_decoder(decoder)
				.map_err(|e| format!("Dynamic image error: {}", e))?;
			Ok(RustImageData::from_dynamic_image(dynamic_image))
		}

		if clipboard_win::is_format_avail(formats::CF_DIBV5) {
			return get_clipboard(formats::RawData(formats::CF_DIBV5))
				.map_err(|e| format!("Get clipboard for CF_DIBV5 error, code = {}", e).into())
				.and_then(|data| handle_dib(&data));
		}

		if clipboard_win::is_format_avail(formats::CF_DIB) {
			return get_clipboard(formats::Bitmap)
				.map_err(|e| format!("Get clipboard error for Bitmap, code = {}", e).into())
				.and_then(|data| handle_dib(&data));
		}
